...
(function(){
	var Provider, nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
			'	<span class="stream-process-bar"><span style="width: 0%;"></span></span>' +'</br></br>'+
			'	登録進捗：<span class="stream-percent">0%</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;残り時間：<strong class="_stream-total-time">&nbsp;</strong>'+
			'  <strong class="_stream-total-uploaded" style="display:none">&nbsp;</strong>' +
			'  <strong class="_stream-total-size" style="display:none">&nbsp;</strong>&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp' +
			'</div>',
	...
	Main.prototype = {
		...
		uploadProgress : function(a) {
			...
			this.costTime = this.costTime + nowTime - this.uploadStartTime;
			this.uploadStartTime = nowTime;
			var myspeed = (this.totalUploadedSize + bytesLoaded)/this.costTime;
		    this.getNode("_stream-total-time", this.totalContainerPanel).innerHTML = this.formatTime(((this.totalFileSize-((this.totalUploadedSize + bytesLoaded)))/myspeed)/1000);
			//end  ------------------------------------------
			this.getNode("_stream-total-uploaded", this.totalContainerPanel).innerHTML = _loaded;
			this.getNode("stream-percent", this.totalContainerPanel).innerHTML = percent + "%";
			this.getNode("stream-process-bar", this.totalContainerPanel).innerHTML = '<span style="width: '+percent+'%;"></span>';
		...
		var total = time || 0, hour = Math.floor(total / 3600), minute = Math.floor((total - 3600 * hour) / 60),
			second = Math.floor(total - 3600 * hour - 60 * minute), hour = "" + (!isNaN(hour) && 0 < hour ? (hour < 10 ? ("0" + hour+"時") : (hour +"時")): ""),
			hour = hour + (!isNaN(minute) && 0 < minute ? (minute < 10 ? (minute +"分") : minute+"分") : "");
		return hour += !isNaN(second) && 0 < second ? (second < 1 ? ("0" + second + "秒") : second+"秒"): "0秒";
	   },
//		formatTime : function(time) {
//			var total = time || 0, hour = Math.floor(total / 3600), minute = Math.floor((total - 3600 * hour) / 60),
