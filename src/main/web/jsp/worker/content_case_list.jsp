...
<div class="page-container">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
<c:forEach items="${listContent}" var="content" varStatus="contentIndex">
				<tr class="text-c">
					<td>${content.contentId}</td>
					<td>${contentIndex.index+1}</td>
					<td>${content.contentTitle}</td>
					<td>${content.carSystemName}</td>
					<td class="td-status" id="status${content.contentId}"><%--<c:if test="${content.status==1}">
					...
					<td>${content.numberRead}</td>
					<td>${content.numberRise}</td>
					<td>${content.numberComment}</td>
					<td>${content.admUserName}</td>
                    <td class="td-time"><fmt:formatDate value="${content.updateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td class="td-manage">

						<textarea id="J_share_text${content.contentId}" class="share-text" style="display: none">${content.contentTitle}</textarea>
						<input class="btn" id="J_copy_clipboard_data${content.contentId}" onclick=" return obj(${content.contentId})" type="button" value="复制链接">
						<c:if test="${content.status==0}">
							<a style="text-decoration:none" onclick="picture_start(this,${content.contentId})" href="javascript:;" title="发布"><i class="Hui-iconfont"></i></a>
						</c:if>
						<c:if test="${content.status==1}">
							<a style="text-decoration:none" onClick="picture_stop(this,'${content.contentId}')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>
						</c:if>
						<a style="text-decoration:none" class="ml-5" onClick="selectContent('设置精选内容','${content.contentId}')" href="javascript:;" title="设置精选内容"><i class="Hui-iconfont">&#xe60c;</i></a>
						<a style="text-decoration:none" class="ml-5" onClick="picture_edit('案例编辑','getContentCaseByContentId.do','${content.contentId}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
						<a style="text-decoration:none" class="ml-5" onClick="picture_del(this,'${content.contentId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
				</tr>
</c:forEach>
			</tbody>
...
$('.table-sort').dataTable({
	...
	"aoColumnDefs": [
	  ...
	  {"orderable":false,"aTargets":[0,9,11]}// 制定列不参与排序
	]
});
/*案例-添加*/
