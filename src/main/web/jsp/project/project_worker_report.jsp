    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <tr class="text-c">
                ...
                <th width="140">日期</th>
                <th width="200">项目进度</th>
                <th width="200">工作计划</th>
                <th width="100">操作</th>
            </tr>
            </thead>
            <tbody>
            <c:forEach items="${report}" var="projectAll" varStatus="projectIndex">
                <tr class="text-c">
                    <td>${projectIndex.index+1}</td>
                    <td onclick="queryDetaila('${projectAll.projectid}')" style="text-decoration:underline">${projectAll.projectname}</td>
                    <td class="td-time"><fmt:formatDate value="${projectAll.date}" pattern="yyyy-MM-dd" /></td>
                    <td>${projectAll.context}</td>
                    <td>${projectAll.plan}</td>
                    <td class="td-manage">
                        <a style="text-decoration:none" class="ml-5 " onClick="picture_query('编辑项目进度','<%=basePath%>/toupdatereport?dataid=${projectAll.dataid}')" title="编辑"><i class="Hui-iconfont">&#xe60c;</i></a></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
