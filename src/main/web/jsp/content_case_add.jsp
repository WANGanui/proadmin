...
<link rel="stylesheet" type="text/css" href="<%=basePath%>style/previewImge.css"/>
<!--[if lt IE 9]>
...
<div class="page-container">
	<form class="form form-horizontal" action="addCaseContent.do" method="post"  id="form-article-add" enctype="multipart/form-data">
		<div class="row cl">
			<%--<input type="hidden" value="${content.contentId}" name="contentId" id="contentId"/>--%>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<select class="select" id="carbrandId"  name="carbrandId" size="1" onchange="queryTwoBarand()">
					<option value="0" > 请选择 </option>
					<%--<c:forEach items="${listCarBrand}" var="carBrand">
						<option value="${carBrand.brandId}">
								${carBrand.brandName}
						</option>
					</c:forEach>--%>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<%--<input type="text" class="input-text" value="${content.contentSummary}" placeholder="" id="contentSummary" name="contentSummary">--%>
				<select class="select" id="carbrandPid"  name="carbrandPid" size="1" onchange="querySystem()">
					<option value="0"> 请选择 </option>
					<%--<c:forEach items="${listCarSystem}" var="carSystem">
						<option value="${carSystem.carSystemId}">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>改装类别：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<%--<input type="text" class="input-text" value="${content.contentSummary}" placeholder="" id="contentSummary" name="contentSummary">--%>
				<select class="select" id="categoryId"  name="categoryId" size="1">
					<option> 请选择 </option>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="" placeholder="" id="contactInfo" name="contactInfo">
				<%--<select class="select" id="carSystem"  name="carSystem" size="1">
					<c:forEach items="${listCategory}" var="category">
						<option value="${category.categoryId}">
								${category.categoryName}
						</option>
		...
		<%--<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">预览封面：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<div class="uploader-list-container">
					<div class="queueList">
						&lt;%&ndash;<div id="dndArea" class="placeholder">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea id="kindeditor4" cols="0" rows="0" name="text"
...
function querySystem() {
	...
	var carbrandId=$("#carbrandPid").val();
	if(carbrandId==0){
		$("#carSystemId").find("option").remove();
		document.getElementById("carSystemId").options.add(new Option("请选择",0));
		return false;
	}
	var  dataJson= {
		brandId:carbrandId
	...
	$.ajax( {
		url : 'getCarSystemByCarBrandId.do',
		type : 'post',
		...
		success : function(data) {
			if (data.success) {
				var listCarSystem= data.listCarSystem;
				var select = document.getElementById("carSystemId");
				// document.write(listRole.length);
				//$("#carbrandPid").options.length=1;
				$("#carSystemId").find("option").remove();
				select.options.add(new Option("请选择",0));
				for(var i = 0; i < listCarSystem.length; i++)
				{

					var id = listCarSystem[i].carSystemId;
					var name = listCarSystem[i].carSystemName;
...
function  queryTwoBarand() {
	...
	if(carbrandId==0){
		...
		document.getElementById("carbrandPid").options.add(new Option("请选择",0));
		return false;
	}
	var  dataJson= {
		brandPid:carbrandId
...
<script type="text/javascript" src="picture_add.js"></script><!--/请在上方写此页面业务相关的脚本-->
</body>
</html>